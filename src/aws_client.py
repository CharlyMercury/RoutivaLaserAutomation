...
from status_access_control import StatusAccessControl
import threading
import sys
import json
...
class AwsMqttClient():
    ...
    def __init__(self, endpoint_, port_, cert_, key_, ca_, client_id_, topics_, parameters, proxy_host=None, proxy_port=None):
        ...
        self.topics = topics_
        self.count = 0
        self.status_access_control = StatusAccessControl()
        self.plantel = parameters["plantel"]
        self.modulo = f'modulo{parameters["modulo"]}'
    ...
    def on_message_received(self, topic, payload, dup, qos, retain, **kwargs):
        ...
        if topic == "status/cbtis0/modulo1":
            # Here goes routine to check status of the module, we are planning to check: internet connection,
            # esp32_status, module status, timestamp
            internet_status, esp_status, timestamp = self.status_access_control.status(self.raspberry)
            message = {
                        "plantel": "cbtis0",
                        "modulo": 1,
                        "status":
            ...
            self.publish_to_topics(topic_pub_="status/response", input_message=message)
